 ...
 */
class Cuentas_Subcategoria_Routes {
    ...
    function load_dependencies(){
        ...
        require_once plugin_dir_path( dirname( __FILE__ ) ) . '/controllers/class-cuentas-subcategoria-controllers.php';

    }

	public function add_endpoints() {
        $_auth = new Cuentas_Auth( $this->plugin_name, $this->version) ;
        $_subcategoria_controllers = new Cuentas_Subcategoria_Controllers( $this->plugin_name, $this->version) ;

		$namespace = 'cuentas/v1';
		$endpoints = array(
			array(
				'endpoint' => 'subcategoria/(?P<categoria_id>\d+)',
				'method' => 'GET',
				'callback' => 'list_all',
				'permission' => 'user_can_read'
			),
			array(
				'endpoint' => 'subcategoria/(?P<categoria_id>\d+)/(?P<subcategoria_id>\d+)',
				'method' => 'GET',
				'callback' => 'list_one',
				'permission' => 'user_can_read'
			),
		...
		foreach ($endpoints as $e) {
			register_rest_route( $namespace, $e['endpoint'], array(
				'methods' => $e['method'],
				'callback' => array($_subcategoria_controllers, $e['callback']),
