...
class Cuentas_Registro_Routes {
    ...
    function load_dependencies(){
        ...
        require_once plugin_dir_path( dirname( __FILE__ ) ) . '/controllers/class-cuentas-registro-controllers.php';

    }

	public function add_endpoints() {
        $_auth = new Cuentas_Auth( $this->plugin_name, $this->version) ;
        $_registro_controllers = new Cuentas_Registro_Controllers( $this->plugin_name, $this->version) ;

		$namespace = 'cuentas/v1';
		$endpoints = array(
			array(
				'endpoint' => 'registro/(?P<cuenta_id>\d+)',
				'method' => 'GET',
				'callback' => 'list_all',
				'permission' => 'user_can_read'
			),
			array(
				'endpoint' => 'registro/(?P<cuenta_id>\d+)/(?P<registro_id>\d+)',
				'method' => 'GET',
				'callback' => 'list_one',
				'permission' => 'user_can_read'
			),
