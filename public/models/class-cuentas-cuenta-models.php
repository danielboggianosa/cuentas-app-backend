...
class Cuentas_Cuenta_Models {
	...
	public function empresa_has_cuenta($empresa, $cuenta){
		global $wpdb;
		$query = "SELECT * FROM cu_cuentas WHERE empresa_id = $empresa AND id = $cuenta;";
		$results = $wpdb->get_results($query, OBJECT);
		if(sizeof($results) > 0)
			return true;
		else
    ...
    public function list_one($usuario, $empresa, $cuenta){
        global $wpdb;
	...
	public function create($usuario_id, $empresa_id, $nueva_cuenta){
		global $wpdb;
		...
		$wpdb->insert('cu_cuentas', $nueva_cuenta);
		...
		if(sizeof($data) > 0){
			...
			return array("success" => false, "error" => "No se pudo crear la cuenta");
		}
	}

	public function update($usuario_id, $empresa_id, $cuenta_id, $data){
		global $wpdb;
		
		if($this->usuario_has_empresa($usuario_id, $empresa_id)){
			$update = $wpdb->update('cu_cuentas', $data, array("id" => $cuenta_id));
			if($update == 0 || !$update){
	...
	public function delete($usuario_id, $empresa_id, $cuenta_id){
		global $wpdb;
